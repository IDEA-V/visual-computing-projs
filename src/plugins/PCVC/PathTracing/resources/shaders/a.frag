...
vec3 tracePath(vec2 px)
{
    ...
    Ray r;
    r.o = (invViewMx * vec4(0.0, 0.0, 0.0, 1.0)).xyz;
    vec4 viewPoint = invViewProjMx * clipPos;
    r.d = viewPoint.xyz/viewPoint.w - r.o;
    r.t = 100.0;
...
{
    // Reseed by iq
    seed = rand + texCoords.y * texCoords.x / iResolution.x + texCoords.y / iResolution.y;
    vec3 color = tracePath(gl_FragCoord.xy);
    fragColor0 = vec4(ltos3(color.x, color.y, color.z), 1);
}